...
/*
 *this class is meant to store and query student data
 */
public class Dbcontroller1 extends SQLiteOpenHelper {
    ...
    public ModelStudent search_stu(String roll) {
        ModelStudent st = new ModelStudent();
        Cursor c = this.getReadableDatabase().rawQuery("SELECT * FROM STU", null);
        while (c.moveToNext()) {
            if (roll.equals(c.getString(1))) {
                st.name = c.getString(2);
                st.cgpa = Integer.parseInt(c.getString(8));
                st.hsc = Integer.parseInt(c.getString(9));
                st.ssc = Integer.parseInt(c.getString(10));
            }
        }
        return st;
    }
